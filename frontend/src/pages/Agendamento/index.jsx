...
function Agendamento() {
  ...
  const proximoPasso = () => {
    if (passo === 2 && !maquinaSelecionada) { toast.warn("Selecione uma máquina!"); return; }
    if (passo === 3 && (!dataSelecionada || !horarioSelecionado)) { toast.warn("Selecione data e horário!"); return; }
    setPasso(passo + 1);
  };
  ...
  const confirmarAgendamento = async () => {
    const user = JSON.parse(localStorage.getItem('usuario'));
    if (!user || !user.id) {
      toast.warn("Usuário não encontrado. Faça login novamente.");
      navigate('/login');
      return;
    }
    setCarregando(true);
      ...
      <main className="main-content">
        <header className="header-home">
          <section className="header-left">
            ...
            <button className="btn-voltar-agendamento" onClick={() => navigate('/home')}><FaArrowLeft /> Voltar</button>
                ...
                <div className="passo-content">
                  <div className="servicos-grid">
                    <div className={`servico-card ${tipoServico === "lavagem" ? 'selected' : ''}`} onClick={() => selecionarTipoServico("lavagem")}>
                      <FaWater className="servico-icon" /><h3>Lavagem</h3><p>Lavagem + Atendimento</p><div className="servico-preco">R$ 30,00</div>
                    </div>
                    <div className={`servico-card ${tipoServico === "secagem" ? 'selected' : ''}`} onClick={() => selecionarTipoServico("secagem")}>
                      <FaWind className="servico-icon" /><h3>Secagem</h3><p>Secagem + Atendimento</p><div className="servico-preco">R$ 20,00</div>
                    </div>
                  </div>
                </div>
              )}
                  ...
                  <div className="maquinas-grid">
                    {(tipoServico === "lavagem" ? lavadoras : secadoras).map((maq) => (
                      <div key={maq.id} className={`maquina-card ${!maq.disponivel || maq.ocupada ? 'indisponivel' : ''} ${maquinaSelecionada?.id === maq.id ? 'selecionada' : ''}`} onClick={() => selecionarMaquina(maq)}>
                        <div className="maquina-icon">{tipoServico === "lavagem" ? <FaTshirt /> : <FaWind />}</div>
                        <div className="maquina-info">
                          <span className="maquina-nome">{maq.nome}</span>
                          <span className="maquina-status">{!maq.disponivel || maq.ocupada ? "Indisponível" : "Disponível"}</span>
                        </div>
                ...
                <div className="passo-content">
                  ...
                  <div className="datas-section">
                    ...
                    <div className="datas-grid">
                      {gerarDatas().map((data, idx) => (
                        <div key={idx} className={`data-card ${dataSelecionada === data ? 'selecionada' : ''}`} onClick={() => setDataSelecionada(data)}>{data}</div>
                      ))}
                    </div>
                  </div>
                  <div className="horarios-section">
                    ...
                    <div className="horarios-grid">
                      {horarios.map((horario) => (
                        <div key={horario} className={`horario-card ${horarioSelecionado === horario ? 'selecionado' : ''}`} onClick={() => setHorarioSelecionado(horario)}>{horario}</div>
                      ))}
                    </div>
                  </div>
                </div>
              ...
              {passo === 4 && (
                <div className="passo-content">
                  ...
                  <div className="confirmacao-card">
                    <div className="confirmacao-row"><span>Serviço:</span><strong>{tipoServico === "lavagem" ? "Lavagem" : "Secagem"}</strong></div>
                    <div className="confirmacao-row"><span>Máquina:</span><strong>{maquinaSelecionada?.nome}</strong></div>
                    <div className="confirmacao-row"><span>Data:</span><strong>{dataSelecionada}</strong></div>
                    <div className="confirmacao-row"><span>Horário:</span><strong>{horarioSelecionado}</strong></div>
                    <div className="confirmacao-row total"><span>Total:</span><strong>R$ {precoTotal.toFixed(2)}</strong></div>
                  </div>
                </div>
              )}

              <div className="botoes-agendamento">
                {passo > 1 && <button className="btn-outline" onClick={voltarPasso}><FaArrowLeft /> Voltar</button>}
                <button className="btn-outline cancel" onClick={() => navigate('/home')}>Cancelar</button>
                {passo < 4 ? (
                  <button className="btn-primary" onClick={proximoPasso}>Próximo →</button>
                ) : (
