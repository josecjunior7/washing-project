...
const Login = () => {
  ...
  const handleRecuperarSenha = async () => {
    if (!emailRecuperar) { toast.warn("Digite seu e-mail!"); return; }
    toast.success(`E-mail de recuperação enviado para ${emailRecuperar}`);
    setModalSenha(false);
    setEmailRecuperar("");
  };

  const handleIrParaCadastro = () => navigate('/cadastro');
      ...
      <form action="">
        <section className="painel-login">
          ...
          <section className="i-senha" style={{ position: "relative" }}>
            ...
            <span onClick={() => setMostrarSenha(!mostrarSenha)} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", cursor: "pointer", color: "#888" }}>
              {mostrarSenha ? (
